...
`write_data_file` <- function(filename,stuck_square){
  ...
  ## square tiles and one compount tile.  If the stuck square is 10x10
  ...
  ## thus a typical last line will look like this (for stuck_square=3):
...
f <- function(stuck_square){
  ...
  write_data_file(filename,stuck_square)
}
